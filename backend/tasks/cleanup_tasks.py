...
def cleanup_via_api(
    self, cleanup_type="unverified_users", days_threshold=7, dry_run=False, force=False
):
    try:
...
def cleanup_unverified_users(self):
    return cleanup_via_api(
        cleanup_type="unverified_users", days_threshold=7, dry_run=False, force=False
    )
...
def weekly_comprehensive_cleanup(self):
    return cleanup_via_api(
        cleanup_type="all", days_threshold=30, dry_run=False, force=True
    )
...
def cleanup_expired_codes(self):
    return cleanup_via_api(
        cleanup_type="expired_codes", days_threshold=30, dry_run=False, force=False
    )
