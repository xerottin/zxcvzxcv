...
async def create_menu_item(
    db: AsyncSession, data: MenuItemCreate, current_user_id: Optional[int] = None
) -> MenuItem:
    try:
        ...
        if "foreign key constraint" in str(e).lower():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid menu reference"
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def delete_menu_item(
    db: AsyncSession, menu_item_id: int, current_user_id: Optional[int] = None
):
    try:
        menu_item = await get_menu_item(db, menu_item_id)
