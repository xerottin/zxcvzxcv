...
async def create_menu_item(db: AsyncSession, data: MenuItemCreate, current_user_id: Optional[int] = None) -> MenuItem:
    try:
        menu_query = select(Menu).where(Menu.id == data.menu_id, Menu.is_active == True)
        result = await db.execute(menu_query)
        menu = result.scalar_one_or_none()

        if not menu:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Active menu with ID {data.menu_id} not found"
            )

        existing_item_query = select(MenuItem).where(
            MenuItem.menu_id == data.menu_id, MenuItem.username == data.username, MenuItem.is_active == True
        )
        ...
        logger.info(f"Menu item created successfully: {new_menu_item.id} " f"by user {current_user_id}")
        ...
        if "unique constraint" in str(e).lower():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Data uniqueness constraint violation")
        if "foreign key constraint" in str(e).lower():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid menu reference")
        else:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Data integrity constraint violation")
...
async def get_menu_item(db: AsyncSession, menu_item_id: int) -> Optional[MenuItem]:
    try:
        ...
        if not menu_item:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Menu item with ID {menu_item_id} not found"
            )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error retrieving menu item")


async def update_menu_item(
    db: AsyncSession, menu_item_id: int, data: MenuItemUpdate, current_user_id: Optional[int] = None
) -> MenuItem:
    try:
        ...
        logger.info(f"Menu item updated successfully: {menu_item.id} " f"by user {current_user_id}")
...
async def delete_menu_item(db: AsyncSession, menu_item_id: int, current_user_id: Optional[int] = None):
    try:
        menu_item = await get_menu_item(db, menu_item_id)
        ...
        if menu_item.is_active:
            ...
            logger.info(f"Menu item soft deleted successfully: {menu_item.id} " f"by user {current_user_id}")
